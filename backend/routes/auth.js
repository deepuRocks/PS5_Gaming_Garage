...
const { OAuth2Client } = require("google-auth-library"); //google client id for google login
const client = new OAuth2Client(process.env.GOOGLE_CLIENT_ID); ////google client id for google login
...
router.put("/profile", async (req, res) => {
  try {
    ...
    const { first_name, last_name, gender, phone, email, country_code } =
      req.body;
  ...
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      const insertResult = await pool.query(
        "INSERT INTO users (first_name, last_name, email, provider, role, password) VALUES ($1,$2,$3,'google','user',$4) RETURNING *",
